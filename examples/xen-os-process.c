...
/**
 ** This is a simple program that probes a stack of two targets: a Xen
 ** VM (via the Xen driver), and a userspace process running inside that
 ** VM (via the os-process driver).  Skip on down to main() so you get
 ** an idea of what's going on; then read through the rest as
 ** necessary.
 **/
...
int main(int argc,char **argv) {
    ...
    /* Connect to the domain specified */
    memset(&bspec,0,sizeof(bspec));
    bspec.domain = argv[1];
    /* Cache up to 512 4096 pages from the target VM. */
    bspec.memcache_mmap_size = 512 * 4096;
    /*
     * If the domain is HVM, you need to tell Stackdb what kernel
     * filename got booted inside it.  If the domain is PV, Stackdb can
     * figure it out from the Xenstore -- but this is not true for HVM.
     */
    //bspec.kernel_filename = strdup("/boot/vmlinuz-2.6.18");

    /* Using the Xen target and software breakpoints only. */
    memset(&tspec,0,sizeof(tspec));
    tspec.target_id = 1;
    tspec.target_type = TARGET_TYPE_XEN;
    /* We force software breakpoints */
    ...
    /*
     * Attach to the target VM.  This leaves the target VM paused.
     */
    ...
    /*
     * Connect to the process specified.  The OS Process driver doesn't
     * require any arguments right now.
     */
    memset(&ospec,0,sizeof(ospec));
    ospec.target_id = 2;
    ospec.target_type = TARGET_TYPE_OS_PROCESS;
    /* We force software breakpoints */
    ospec.style = PROBEPOINT_SW;
    ospec.backend_spec = NULL;

    /*
     * If our domain does not use the same binaries as dom0, we'll have
     * to download the debuginfo for it and put it in dom0's filesystem
     * in a directory of its own.  This is the path to that directory.
     */
    //ospec.debugfile_root_prefix = strdup("/path/to/debuginfo");

    /* Look up the process we want, by PID or name. */
    if (isdigit(*(argv[2])))
	otid = target_lookup_overlay_thread_by_id(target,atoi(argv[2]));
    else
	otid = target_lookup_overlay_thread_by_name(target,argv[2]);

    /* Instantiate the overlay target object! */
    overlay = target_instantiate_overlay(target,otid,&ospec);
    if (!overlay) {
    ...
    /*
     * Get a descriptive name for the overlay target.
     */
    target_snprintf(overlay,otargetstr,sizeof(otargetstr));

    /*
     * Attach to the target program in the VM.  This leaves the target
     * program paused.
     */
    if (target_open(overlay)) {
	fprintf(stderr,"could not open %s!\n",otargetstr);
	cleanup();
	exit(-4);
    }
    ...
    /*
     * Place probes on symbols.  Try looking up first in the base
     * target; then in the overlay target.
     */
    for (i = 3; i < argc; ++i) {
	struct target *t;

	/*
	 * Look up a symbol in the overlay target!
	 */
	t = target;
	name = argv[i];
	bsymbol = target_lookup_sym(t,name,NULL,NULL,SYMBOL_TYPE_FLAG_NONE);
	if (!bsymbol) {
	    t = overlay;
	    bsymbol = target_lookup_sym(t,name,NULL,NULL,SYMBOL_TYPE_FLAG_NONE);
	    if (!bsymbol) {
	 ...
	 * above!
	 */
	probe = probe_value_symbol(t,TID_GLOBAL,bsymbol,handler,handler,NULL);
	if (!probe) {
