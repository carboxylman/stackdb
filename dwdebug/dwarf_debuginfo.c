...
static struct location *dwarf_get_loclistloc(struct symbol_root_dwarf *srd,
					     unsigned int attr,
					     Dwarf_Word offset);
static struct location *dwarf_get_static_ops(struct symbol_root_dwarf *srd,
					     Dwarf_Word len,
...
#define DAWL(msg)						\
	...
	if (block_set) {
	    if (symbol->ismember) {
		loc = dwarf_get_static_ops(srd,block.length,block.data,attr);
		if (!loc) {
...
static struct location *dwarf_get_loclistloc(struct symbol_root_dwarf *srd,
	...
	if (begin == (Dwarf_Addr)-1l) {
	    ...
	    tmploc = dwarf_get_static_ops(srd,exprlen,
					  (unsigned char *)readp,attr);
...
#define OPCONSTS(size,tt)						\
	...
	*/
    ...
    vdebug(6,LA_DEBUG,LF_DWARFOPS,"had to save dwarf ops for runtime!\n");
    location_set_runtime(retval,(char *)origdata,origlen);
