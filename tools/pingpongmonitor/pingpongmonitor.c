...
/**
 ** This program is mostly the same as the multi-base example.
 ** The only different is that the output is following the format required by
 ** the RPC java client.
 ** The input arguments is just the same as multi-base. So you need to 
 ** construct program parameters as below
 ** 
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "Starting main debugging loop!\n");
    //fflush(stdout);
    ...
    fprintf(stderr,"Monitoring completed; exiting!\n");
...
result_t handler_line(struct probe * probe, tid_t tid, void *data,
    ...
    if (value) {
    	...
    	snprintf(outbuf,sizeof(bsymbol_get_name(psym))+sizeof(buf)+21,
                "Value of %s is now '%s'", bsymbol_get_name(psym), buf);
        // I just print out the required format but do make use of most of the 
        // fields, because in this example I just want to print out values I get
		fprintf(stdout,"RESULT:: (%c:%d) %s (%d) %s \"%s\" () ::RESULT\n", 
                'r',1,"--",1,"--", outbuf);
    }
